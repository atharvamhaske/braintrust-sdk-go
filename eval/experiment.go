...
//
// projectID takes precedence over projectName. When it is set the project is
// used as-is; otherwise the project is created (or fetched) by name, which
// requires create permission. The Braintrust playground supplies an ID
// precisely so that a caller's token does not need that permission.
func registerExperiment(ctx context.Context, apiClient *api.API, name string, projectName string, projectID string, tags []string, metadata map[string]interface{}, update bool, dataset datasetInfo) (*experiments.Experiment, error) {
	if name == "" {
	...
	experiment, err := apiClient.Experiments().Register(ctx, name, projectID, experiments.RegisterOpts{
		Tags:           tags,
		Metadata:       metadata,
		Update:         update,
		DatasetID:      datasetID,
