...
func TestGenerateExcludesAllDirectory(t *testing.T) {
	...
	// Count aspects - should be exactly 20 (one per provider, plus openai-v2, 4 for adk, 6 for genkit, 1 for cloudwego/eino, 1 for bedrockruntime, and 2 for mcp)
	// If it's more, we might be including duplicates from all/
	if len(result.Aspects) != 20 {
		t.Errorf("expected 20 aspects, got %d", len(result.Aspects))
	}
}
