...
// input_json_delta must only supply a block type when content_block_start did
// not, and the accumulated JSON must decode into input as an object.
func TestClaudeStreamAccumulatorToolInputBlocks(t *testing.T) {
	tests := []struct {
		...
		{
			// No deltas: the empty string must not replace the object input.
		...
		{
			// No type to preserve, so the delta supplies the default.
